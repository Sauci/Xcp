...
class XcpTest(object):
    ...
    def __init__(self,
        ...
        # SP5-NV Task 4 (design doc DD100, docs/superpowers/specs/2026-09-09-xcp-daq-nv-storage-
        # design.md): same reasoning as xcp_store_daq_configuration/xcp_clear_daq_configuration
        # immediately above -- Xcp_ReadStoredSessionConfigurationId is declared unconditionally in
        # interface/Xcp.h, not behind xcp_read_stored_session_configuration_id_api_enable (that
        # flag gates Xcp_Init arming the poll at runtime, through Xcp_GeneralType, not the
        # declaration itself) -- so this reaches self.code.mocked in every configuration this
        # harness builds and must be supplied unconditionally here.
        self.xcp_read_stored_session_configuration_id = MagicMock()
        ...
        # Task 5 (PROGRAM_PREPARE): same reasoning as xcp_program_reset immediately above --
        # Xcp_ProgramPrepare is declared in interface/Xcp.h only (not mirrored into
        # test/stub/Xcp_MemoryAccess.h; design §4 -- cffi's cdef() rejects a second declaration
        # reachable from the same header with FFIError: multiple declarations), so this
        # constructor must supply the attribute unconditionally rather than let it depend on which
        # build a given test happens to be.
        self.xcp_program_prepare = MagicMock()
        # Task 2 (PROGRAM_CLEAR): same reasoning as xcp_program_prepare immediately above --
        # Xcp_ProgramClear is declared in interface/Xcp.h only (not mirrored into
        ...
        # constructor must supply the attribute unconditionally rather than let it depend on which
        # build a given test happens to be.
        self.xcp_program_verify = MagicMock()
        # SP4c Task 3 (PROGRAM_FORMAT): same reasoning as xcp_program_verify immediately above --
        # Xcp_ProgramFormat is declared in interface/Xcp.h only (not mirrored into
        # test/stub/Xcp_MemoryAccess.h; design Section 4 -- cffi's cdef() rejects a second
        # declaration reachable from the same header with FFIError: multiple declarations), so this
        # constructor must supply the attribute unconditionally rather than let it depend on which
        # build a given test happens to be.
        self.xcp_program_format = MagicMock()
        ...
        # Harmless even though DefaultConfig() itself defaults the matching API flag off (test/
        # parameter.py), which keeps Xcp_Init from ever arming the poll that would call this: set
        # for the same blanket-default hygiene as every mock on this and the surrounding lines.
        self.xcp_read_stored_session_configuration_id.return_value = self.define('E_OK')
