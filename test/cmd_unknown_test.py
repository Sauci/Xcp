...
@pytest.mark.parametrize('pid', tuple(pid for pid in range(0xC0, 0xE4) if pid not in (0xE1, 0xE2, 0xE3)))
def test_unimplemented_commands_return_err_cmd_unknown(pid):
    ...
    0xE1 (WRITE_DAQ), 0xE2 (SET_DAQ_PTR) and 0xE3 (CLEAR_DAQ_LIST) are excluded: all three are
    implemented now, and write_daq_test.py, set_daq_ptr_test.py and clear_daq_list_test.py cover
    them."""
