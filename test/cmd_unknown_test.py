...
def test_unimplemented_commands_return_err_cmd_unknown(pid):
    ...
    0xD7 (GET_DAQ_EVENT_INFO), 0xD8 (GET_DAQ_LIST_INFO), 0xD9 (GET_DAQ_RESOLUTION_INFO), 0xDA
    (GET_DAQ_PROCESSOR_INFO), 0xDB (READ_DAQ), 0xDD (START_STOP_SYNCH), 0xDE
    (START_STOP_DAQ_LIST), 0xDF (GET_DAQ_LIST_MODE), 0xE0 (SET_DAQ_LIST_MODE), 0xE1 (WRITE_DAQ),
    0xE2 (SET_DAQ_PTR) and 0xE3 (CLEAR_DAQ_LIST) are excluded: all twelve are implemented now, and
    get_daq_event_info_test.py, get_daq_list_info_test.py, get_daq_resolution_info_test.py,
