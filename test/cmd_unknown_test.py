...
@pytest.mark.parametrize('pid', tuple(pid for pid in range(0xC0, 0xE4) if pid not in (0xD9, 0xDA, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3)))
def test_unimplemented_commands_return_err_cmd_unknown(pid):
    ...
    0xD9 (GET_DAQ_RESOLUTION_INFO), 0xDA (GET_DAQ_PROCESSOR_INFO), 0xDD (START_STOP_SYNCH), 0xDE
    (START_STOP_DAQ_LIST), 0xDF (GET_DAQ_LIST_MODE), 0xE0 (SET_DAQ_LIST_MODE), 0xE1 (WRITE_DAQ),
    0xE2 (SET_DAQ_PTR) and 0xE3 (CLEAR_DAQ_LIST) are excluded: all nine are implemented now, and
    get_daq_resolution_info_test.py, get_daq_processor_info_test.py, start_stop_synch_test.py,
