...
| ```daq_queue_size```            | 16      | how many sampled DTO frames may wait for transmission at once. `Xcp_TriggerEventChannel` fills the ring; the transmission chain drains one frame per `CanIf` confirmation, so this sizes the burst the slave can absorb when sampling briefly outruns the bus. Each slot costs `MAX_DTO` bytes plus a few, and a frame sampled while the ring is full is dropped and reported per `overload_indication` |
