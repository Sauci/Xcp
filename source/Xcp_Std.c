...
uint8 Xcp_DTOCmdStdSetRequest(boolean *responseExpected, const PduInfoType *pPduInfo)
{
    ...
    *responseExpected = TRUE;
     ...
     *
     * Both bits are in the accepted mask now, under the same gate. Design doc DD103-DD107
     * (docs/superpowers/specs/2026-09-10-xcp-daq-resume-design.md, SP5-RESUME) implements RESUME
     * mode, and GET_DAQ_PROCESSOR_INFO now reports RESUME_SUPPORTED set unconditionally
     * (Xcp_DTOCmdDaqGetDaqProcessorInfo, source/Xcp_Daq.c) -- refusing bit 2 while advertising the
     * capability it arms would be the identical incoherence the other way round, a master told
     * the slave offers a mode its own SET_REQUEST then refuses to let it ask for. What bit 2 arms
     * is still gated on whether there is anywhere to store to, exactly as bit 1 is: neither DAQ
     * store mode is accepted unless storeDaqConfigurationApiEnable says a non-volatile store
     * exists (see the paragraph below); an unconfigured build refuses both, through this section's
     * own "If the slave device does not support the requested mode, an ERR_OUT_OF_RANGE will be
     * returned" escape hatch, with no branch of its own. RESUME_SUPPORTED is not conditioned on
     * that same flag: design doc DD104 notes that Xcp_ResumeComplete and the Xcp_Restore* setters
     * it commits are reached only through the integrator's own calls, never through this handler,
     * so an unconfigured build can still genuinely resume a configuration however its non-volatile
     * memory came to hold one -- the flag gates SET_REQUEST's own live store, not the slave's
     * general capacity to start in RESUME mode.
     *
     * STORE_CAL_REQ is unconditional: Xcp_MainFunction fulfils it through the integrator's
     * store-calibration callback, then clears the bit and raises EV_STORE_CAL. STORE_DAQ_REQ_
     * NO_RESUME, STORE_DAQ_REQ_RESUME and CLEAR_DAQ_REQ each act on a non-volatile DAQ list
     * configuration this module keeps only when the matching integrator callback is configured in
     * -- Xcp_StoreDaqConfiguration and Xcp_ClearDaqConfiguration (interface/Xcp.h), polled from
     * Xcp_MainFunction on the identical contract STORE_CAL_REQ's own callback follows. Each mode
     * bit is therefore folded into the accepted mask below only when its own
     * xcp_store_daq_configuration_api_enable / xcp_clear_daq_configuration_api_enable flag is
     * enabled (Xcp_Ptr->general, generated by script/source_cfg.c.jinja2); an unconfigured build
     * has nothing to fulfil any of the three and refuses them under this same section's
     * "If the slave device does not support the requested mode, an ERR_OUT_OF_RANGE will be
     * returned" -- exactly as it always has.
     *
     ...
     *
     * The two bit tables do NOT coincide under 1.1: SET_REQUEST's store request is mode bits 1 AND
     * 2, while the pending flag GET_STATUS reports is the single session status bit 2 (1.1/
     * 1.6.1.1.3) -- both DAQ store modes report through that one flag, which is why the
     * translation below sets it from either bit rather than reserving a session-status bit per
     * mode. It translates mode bits into session status bits one at a time instead of OR-ing the
     * received byte, which is what it used to do and what only worked while 1.0's tables lined up.
     * Xcp_GetResumeArmedState's own flag (interface/Xcp.h, design doc DD104) is a THIRD table,
     * distinct from both of the above: it is what tells the two DAQ store modes apart, which
     * session status's one shared bit cannot. */
    if (Xcp_Ptr->general->storeDaqConfigurationApiEnable == TRUE)
    {
        accepted_request_mask |= XCP_SET_REQUEST_MODE_STORE_DAQ_REQ_NO_RESUME;
        accepted_request_mask |= XCP_SET_REQUEST_MODE_STORE_DAQ_REQ_RESUME;
    }
    ...
    if (result == E_OK)
    {
        ...
        if ((pPduInfo->SduDataPtr[0x01u] & XCP_SET_REQUEST_MODE_STORE_DAQ_REQ_NO_RESUME) != 0x00u)
        {
            ...
            /* Design doc DD104: "does not set the slave into RESUME mode" (1.1/1.6.1.2.3),
             * written here rather than left at whatever an earlier request left it, so
             * Xcp_GetResumeArmedState answers this request's own mode rather than a stale one. */
            Xcp_Internal.resume_armed = FALSE;
        }

        if ((pPduInfo->SduDataPtr[0x01u] & XCP_SET_REQUEST_MODE_STORE_DAQ_REQ_RESUME) != 0x00u)
        {
            /* Same session status bit as STORE_DAQ_REQ_NO_RESUME above -- 1.1's GET_STATUS keeps
             * one shared STORE_DAQ_REQ flag (this function's own top comment) -- but resume_armed
             * is what tells Xcp_StoreDaqConfiguration the two modes apart (design doc DD104). */
            Xcp_Internal.session_status |= XCP_SESSION_STATUS_MASK_STORE_DAQ_REQ;
            Xcp_Internal.resume_armed = TRUE;
        }
