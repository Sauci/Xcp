...
static const uint32 Xcp_CRC32Table[] = {
    0xFFFFFFFFu, 0x77073096u, 0xEE0E612Cu, 0x990951BAu, 0x076DC419u, 0x706AF48Fu, 0xE963A535u, 0x9E6495A3u,
...
static uint8 Xcp_DTOCmdStdBuildChecksum(PduIdType rxPduId, const PduInfoType *pPduInfo)
{
    ...
    if (block_size > 0x00u)
    {
        upper_address = Xcp_Rt.memory_transfer_address.address + (element_size * block_size * 0x08u);

        remainder = 0x00000000u;

        for (; Xcp_Rt.memory_transfer_address.address < upper_address; Xcp_Rt.memory_transfer_address.address+= 0x08u)
        {
            Xcp_ReadSlaveMemoryU8(Xcp_Rt.memory_transfer_address.address, Xcp_Rt.memory_transfer_address.extension, &data);
            remainder = (remainder >> 0x08u) ^ Xcp_CRC32Table[(remainder ^ data) & 0xFFu];
        }
        ...
        /* TODO: At the moment, only CRC32 algorithm is supported. This value should be dynamically set if others algorithm support are added... */
        Xcp_Rt.cto_response.pdu_info.SduDataPtr[0x01u] = 0x09u;

        Xcp_Rt.cto_response.pdu_info.SduDataPtr[0x02u] = 0x00u;
        Xcp_Rt.cto_response.pdu_info.SduDataPtr[0x03u] = 0x00u;

        Xcp_CopyFromU32WithOrder(remainder, &Xcp_Rt.cto_response.pdu_info.SduDataPtr[0x04u], Xcp_Ptr->general->byteOrder);
    }
    else
    {
