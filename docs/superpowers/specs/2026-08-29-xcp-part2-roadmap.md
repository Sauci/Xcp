...
| Tests | `test/*_test.py` — pytest + CFFI compiling the real C, 13073 passing, 29 skipped. `test.sh` reports coverage as the union across compilation variants (`script/gcov_union.py`), since build-time guards make one source several structurally different programs |
...
| Event codes (EV_*) | §1.2 | `EV_STORE_CAL` (0x03) and `EV_DAQ_OVERLOAD` (0x06), the latter added in SP2a and configurable through `overload_indication`. `EV_CLEAR_DAQ` (0x01) and `EV_STORE_DAQ` (0x02) added in SP5-NV. `EV_RESUME_MODE` (0x00) added in SP5-RESUME, queued by `Xcp_ResumeComplete`. `EV_CMD_PENDING` was also listed absent here, which was already stale independent of this row's own RESUME correction — this row's own §2.6 neighbour above has read `done — shipped in SP4` since before SP5-RESUME existed. **Absent: two of the thirteen codes 1.1/§1.2 defines** — `EV_SLEEP` (0x0A) and `EV_WAKE_UP` (0x0B), which are §1.8.10's SLEEP mode rather than packets. `EV_TIME_SYNC` (0x08) shipped 2026-09-17 as `Xcp_RaiseTimeSyncEvent`, which the integrator calls from their sync-line handler with a timestamp captured at the edge — the module does not read the clock itself, since it has none of its own (`Xcp_GetDaqTimestamp` is integrator-supplied) and sampling at call time would discard exactly the accuracy §1.8.8 exists for. Design: `docs/superpowers/specs/2026-09-17-xcp-time-sync-design.md` (DD154–DD157). **§1.8.8 contradicts itself on the timestamp's width** — its table says DWORD, its prose says the format is `GET_DAQ_RESOLUTION_INFO`'s, which encodes a size of 1, 2 or 4 bytes. DD154 takes the table, so a `ONE_BYTE` configuration sends the full 32 bits in this event while its DAQ frames carry a truncated timestamp. That is deliberate and pinned by test; it is not a defect to be tidied. `EV_STIM_TIMEOUT` (0x09) shipped 2026-09-17: `Xcp_TriggerEventChannel` counts consecutive stimulation events at which a STIM DAQ list received nothing, and raises it at a threshold each event channel declares for itself (absent = never). Design: `docs/superpowers/specs/2026-09-17-xcp-stim-timeout-design.md` (DD149–DD153) — §1.8.9 defines the packet and none of the policy, so every part of that is recorded there as a choice. **Its Info Type 1 row, `1 = DAQ list number`, appears nowhere in the 1.1 OCR sidecar** and was recovered from the PDF's own enciphered text layer by the known-plaintext method of `2026-09-11-xcp-get-id-types-design.md`; a design written from the sidecar would have implemented one Info Type believing that was all §1.8.9 defines. That is the second time this sidecar has been wrong about this table family. It was seven until 2026-09-17: `EV_SESSION_TERMINATED` (0x07), `EV_USER` (0xFE) and `EV_TRANSPORT` (0xFF) now ship, through `Xcp_TerminateSession`, `Xcp_RaiseUserEvent` and `Xcp_RaiseTransportEvent`. The two carriers cost almost nothing once DD138 had taught the event queue's transmit branch to lay `userData` out at 2..MAX_CTO-1 for `SERV_TEXT`, which is where §1.1.3.4 puts an EV packet's information data; `2026-09-16-xcp-serv-codes-design.md` scoped them out for having no caller rather than for lacking machinery. `EV_TRANSPORT` is carried and never interpreted — §1.2 refers its content to Part 3, which is not in `docs/external`. `Xcp_TerminateSession` both queues the event and disconnects, because §1.2 makes the code the announcement of a decision already taken and `Xcp_DisconnectSession` is internal. **The four that remain are not merely packets**, which is why they are not grouped with these three: §1.8.10 gives `EV_SLEEP`/`EV_WAKE_UP` a SLEEP mode, and §1.8.8's `EV_TIME_SYNC` an externally triggered sync line — see the note below. The original absent set, all seven, was `EV_SESSION_TERMINATED` (0x07), `EV_TIME_SYNC` (0x08), `EV_STIM_TIMEOUT` (0x09), `EV_SLEEP` (0x0A), `EV_WAKE_UP` (0x0B), `EV_USER` (0xFE) and `EV_TRANSPORT` (0xFF); §1.2 opens by making the whole table optional. This row previously listed three — 0x07, 0xFE, 0xFF — which is **exactly 1.0/§1.2's absent set**: 1.0's table ends at 0x07 and 1.1 adds 0x08–0x0B. That is the revision rule at the head of this document, missed on a cross-cutting table rather than on a command. Corrected 2026-09-15 from the 1.1 PDF's **own text layer**, deciphered by the known-plaintext method of §0 of `2026-09-11-xcp-get-id-types-design.md` and validated by decoding page footers back to their own printed numbers — not from the OCR sidecar, which garbles this table's code column. `EV_STIM_TIMEOUT` was already tracked, deferred by name out of SP3 (§4); 0x08, 0x0A and 0x0B were tracked nowhere. None of the three is only a packet: 1.1/§1.8.10 gives `EV_SLEEP`/`EV_WAKE_UP` a SLEEP mode where the slave stays CONNECTED, processes no commands and sends neither `ERR_CMD_BUSY` nor `EV_CMD_PENDING`, so shipping the two events means shipping that mode; and 1.1/§1.8.8's `EV_TIME_SYNC` reports a DWORD timestamp in `GET_DAQ_RESOLUTION_INFO` format on an externally triggered sync line, withdrawn only from a slave that has no timestamps — this module has had them since SP2b. **Every one of them was transmitted as a zero-length frame until 2026-09-16**, so no event code ever reached the bus: `Xcp_TransmitOneFrame` wrote the EV packet's two bytes and handed the PduInfo to CanIf without setting `SduLength`, which was assigned exactly once in the module -- to 0, at `Xcp_Init`. That includes `EV_CMD_PENDING`, which SP4 built precisely so a master restarts its time-out during deferred programming and which it cannot do from an empty frame. The suite was green throughout because every event assertion read `SduDataPtr` and none read `SduLength`, the same blind spot behind D18's Finding 4 and behind `GET_SEED` and `SHORT_UPLOAD`'s missing finalizes; found by asking the event path the question DD133's invariant asks of the CTO path, which covers that buffer only. Fixed by finalizing through the same helper every CTO response uses, with length assertions added to the event tests so the blind spot closes rather than moves (`test/event_frame_length_test.py`) |
...
>
> **The DAQ transmit path was audited too, and is sound (2026-09-16).** Recorded because it is a
> negative result and would otherwise be re-investigated. `Xcp_DaqRuntime.c:1012` sets `SduLength`
> from the queued frame's own `length`; that length is `Xcp_DaqWriteIdentificationField`'s return
> value plus the timestamp plus the elements copied (`:416`), so the identification field is
> counted rather than omitted; and an ODT that copies no element returns `E_NOT_OK` (`:318`), which
> `:1203` checks before enqueuing, so a header-only frame is never transmitted. The third writer of
> a `length` in that file, `:984`, is the STIM receive path and not a transmit length. With the CTO
> buffer guarded and the event buffer fixed, all three transmit paths are accounted for.
>
...
corrected here. What is left of SP5 is its residue: the interleaved communication model, the seven
remaining `EV_*` codes and the two `SERV_*` codes — `GET_ID` identification types 1–4/128–255
were the third residue item until SP5-GETID shipped them. This sentence dropped the `EV_*` codes
altogether, and §2.6's own event row undercounted them; both corrected 2026-09-15.**
...
The `SERV_*` service request codes were listed here and are now **done** — both shipped on 2026-09-16; see §2.6's row and `2026-09-16-xcp-serv-codes-design.md`. **That leaves the interleaved communication model as SP5's only remaining item**, and it is the last entry on this roadmap that is neither done nor deliberately removed.
